...
// הגדרת משתני מצב גלובליים למניעת ReferenceError בפתיחת תפריטים (סעיף א', ב')
...
function loadLocalBackupData() {
    ...
    window.teamMessages = [{ id: "msg_1", from: "מערכת", to: "כולם", text: "ברוכים הבאים למערכת המשותפת!", date: "8.7.2026", archive: false, readBy: [] }];
}
...
// מבנה כפתורים מצומצם ומאוזן לפי המפרט הויזואלי המדויק: [▲] [+] [ קלט ] [-] (סעיף ב')
function createQtyControllerHtml(category, origIndex, field, currentValue) {
    return `
        <div class="flex flex-row items-center justify-center gap-1 bg-slate-50 dark:bg-slate-950 p-1 rounded-xl border dark:border-slate-700 mx-auto max-w-[150px]">
            <button onclick="window.updateItemValue('${category}', ${origIndex}, '${field}', ${currentValue + 1})" class="w-6 h-6 text-xs bg-white dark:bg-slate-700 border dark:border-slate-600 rounded font-black shadow-sm text-slate-700 dark:text-white" title="הוסף 1">▲</button>
            <button onclick="window.updateItemValue('${category}', ${origIndex}, '${field}', ${currentValue + 0.5})" class="w-6 h-6 text-xs bg-white dark:bg-slate-700 border dark:border-slate-600 rounded font-black shadow-sm text-slate-700 dark:text-white" title="הוסף 0.5">+</button>
            ...
            <button onclick="window.updateItemValue('${category}', ${origIndex}, '${field}', ${currentValue - 0.5})" class="w-6 h-6 text-xs bg-white dark:bg-slate-700 border dark:border-slate-600 rounded font-black shadow-sm text-slate-700 dark:text-white" title="הורד 0.5">-</button>
        </div>
    `;
}
...
function renderApp() {
    ...
    for (const [catName, items] of Object.entries(window.appData)) {
        ...
        const filtered = itemsWithMeta.filter(item => {
            ...
            // פתרון באג סינון ימי שימוש - מציג מוצרים המשמשים לכל הימים תמיד (סעיף ה')
        ...
        if (window.viewMode === 'grid') {
            ...
            filtered.forEach(item => {
                ...
                rowsHtml += `
                    <tr class="table-row-floating border-b dark:border-slate-700 text-xs font-bold hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition">
                        ...
                        <!-- הסרת כפתורי הפלוס והמינוס מעמודת יעד מומלץ (סעיף ב') -->
            ...
            // מחיקת עמודת לחתונה והרחבת עמודת ימי השימוש (סעיף ד')
                ...
                <table class="w-full text-right border-separate border-spacing-0 custom-table">
                    <thead>
                        <tr class="text-[11px] font-black text-slate-400 bg-slate-50/80 dark:bg-slate-700/50">
                            ...
                            <th class="p-3 text-center w-40">ימי שימוש ותפריט מורחב</th>
...
function setMessageCenterTabUI() {
    ...
    document.getElementById('msg-tab-received').className = tab === 'received' ? "px-2 py-1 rounded bg-white dark:bg-slate-600 shadow-sm text-slate-800 dark:text-white font-black" : "px-2 py-1 rounded text-slate-500 dark:text-slate-300";
    document.getElementById('msg-tab-sent').className = tab === 'sent' ? "px-2 py-1 rounded bg-white dark:bg-slate-600 shadow-sm text-slate-800 dark:text-white font-black" : "px-2 py-1 rounded text-slate-500 dark:text-slate-300";
    document.getElementById('msg-tab-archive').className = tab === 'archive' ? "px-2 py-1 rounded bg-white dark:bg-slate-600 shadow-sm text-slate-800 dark:text-white font-black" : "px-2 py-1 rounded text-slate-500 dark:text-slate-300";
}

// מערכת ההודעות המפוצלת המכילה מחיקה וארכוב (סעיף ג')
function renderMessages() {
    ...
    let unreadCount = 0;
    window.teamMessages.forEach(m => { if ((m.to === "כולם" || m.to === window.currentUser.name) && !m.readBy.includes(window.currentUser.name) && m.from !== window.currentUser.name && !m.archive) unreadCount++; });
    ...
    if (window.messageCenterTab === 'received') {
        displayList = window.teamMessages.filter(m => (m.to === "כולם" || m.to === window.currentUser.name) && !m.archive);
    } else if (window.messageCenterTab === 'sent') {
        displayList = window.teamMessages.filter(m => m.from === window.currentUser.name && !m.archive);
    } else {
        displayList = window.teamMessages.filter(m => m.archive === true);
    }
    ...
    displayList.forEach(m => {
        ...
        item.className = "p-2 border-b dark:border-slate-700 text-[11px] hover:bg-slate-50 flex justify-between items-start gap-1";
        item.innerHTML = `
            <div class="flex-1">
            ...
            <div class="flex flex-col gap-1">
                <button class="text-red-400 hover:text-red-600 font-bold text-[10px] del-msg-btn" title="מחק לצמיתות">🗑️</button>
                ${!m.archive ? `<button class="text-amber-500 hover:text-amber-600 font-bold text-[10px] arc-msg-btn" title="העבר לארכיון">📦</button>` : ''}
            </div>
        `;
        item.querySelector('.del-msg-btn').onclick = () => deleteMessageComplete(m.id);
        if (!m.archive) {
            item.querySelector('.arc-msg-btn').onclick = () => archiveMessageComplete(m.id);
        }
...
function archiveMessageComplete(id) {
    let msg = window.teamMessages.find(m => m.id === id);
    if (msg) { msg.archive = true; renderMessages(); triggerDebouncedSync(true); showToast("ההודעה הועברה לארכיון"); }
}
function deleteMessageComplete(id) { window.teamMessages = window.teamMessages.filter(m => m.id !== id); renderMessages(); triggerDebouncedSync(true); showToast("ההודעה נמחקה לצמיתות"); }
...
function toggleFloatingChat() { 
    ...
    if (window.isChatOpen) { win.classList.remove('hidden'); renderChatMessages(); } else { win.classList.add('hidden'); } 
}
window.toggleFloatingChat = toggleFloatingChat;

function sendChatMessage() { const inp = document.getElementById('chat-text-input'); const text = inp.value.trim(); if (!text || !window.currentUser) return; const target = "כולם"; window.teamMessages.unshift({ id: "msg_" + Date.now(), from: window.currentUser.name, to: target, text, date: new Date().toLocaleDateString('he-IL'), archive: false, readBy: [window.currentUser.name] }); inp.value = ''; renderApp(); triggerDebouncedSync(true); }
window.sendChatMessage = sendChatMessage;

function renderChatMessages() { const container = document.getElementById('chat-messages-container'); if (!container || !window.currentUser) return; container.innerHTML = ''; window.teamMessages.filter(m => !m.archive).forEach(m => { container.innerHTML += `<div class="p-2 border rounded-xl bg-white mb-1 shadow-sm text-slate-800 dark:bg-slate-700 dark:text-white dark:border-slate-600"><div class="text-[9px] text-slate-400"><b>${m.from}</b></div><p>${m.text}</p></div>`; }); }
...
// הוספת מנגנון ייצוא הורדה וטעינה חיצונית של קובץ JSON (סעיף ו')
function exportData(type) {
    if (type === 'download') {
        const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify({
            appData: window.appData, teamMembers: window.teamMembers, teamMessages: window.teamMessages, vegetableMatrix: window.vegetableMatrix, toolMatrix: window.toolMatrix
        }));
        const dlAnchor = document.createElement('a'); dlAnchor.setAttribute("href", dataStr); dlAnchor.setAttribute("download", "ali_siach_backup.json"); dlAnchor.click();
        showToast("קובץ גיבוי הורד בהצלחה!", "💾"); toggleSharePopover(); return;
    }
...
function importBackupFile(e) {
    ...
    reader.onload = function(evt) {
        try {
            ...
            if (parsed.appData && parsed.teamMembers) {
                window.appData = parsed.appData; window.teamMembers = parsed.teamMembers; window.teamMessages = parsed.teamMessages || [];
                if (parsed.vegetableMatrix) window.vegetableMatrix = parsed.vegetableMatrix; if (parsed.toolMatrix) window.toolMatrix = parsed.toolMatrix;
                renderApp(); triggerDebouncedSync(true); showToast("הנתונים שוחזרו בהצלחה!", "✅"); toggleSettingsModal();
            } else { alert("מבנה קובץ גיבוי לא תקין!"); }
        } catch(err) { alert("שגיאה בפענוח קובץ הגיבוי!"); }
    };
