...
function loadLocalBackupData() {
    ...
    window.teamMessages = [{ id: "msg_1", from: "מערכת", to: "כולם", text: "ברוכים הבאים למערכת המשותפת!", date: "8.7.2026", readBy: [], isArchived: false }];
}
...
function renderChatMessages() { 
    ...
    displayList.forEach(m => {
        ...
        item.querySelector('.msg-del-btn').onclick = () => deleteMessageComplete(m.id);
        if (!m.isArchived) {
            item.querySelector('.msg-arc-btn').onclick = () => archiveMessageComplete(m.id);
        }
        container.appendChild(item);
    });
}
...
function exportData(type) {
    ...
    if (type === 'download') {
        ...
        const backupObj = {
            ...
            teamMessages: window.teamMessages,
            vegetableMatrix: window.vegetableMatrix,
            toolMatrix: window.toolMatrix
        };
