...
function loadLocalBackupData() {
    ...
    window.appData = {
        "טואלטיקה וניקיון": [
            { name: "אבקת כביסה", existing: 1, recommended: 3, price: 39, orderedLastMonth: 2, notes: "לכביסה", days: "שני, חמישי", changesCount: 4 },
            { name: "אמה (נוזל כלים)", existing: 3, recommended: 3, price: 7.2, orderedLastMonth: 3, notes: "", days: "כל הימים", changesCount: 1 }
        ],
        "מוצרים יבשים ושימורים": [
            { name: "אורז פרסי", existing: 4, recommended: 5, price: 13, orderedLastMonth: 5, notes: "שבת", days: "שישי, שבת", changesCount: 2 },
            { name: "פסטה", existing: 2, recommended: 4, price: 6, orderedLastMonth: 4, notes: "צהריים", days: "ראשון, שלישי", changesCount: 8 }
        ]
    };
    window.teamMembers = [
...
function updateChartData() {
    if (!window.myChart) return; let labels = [], data = [];
    for (const [cat, items] of Object.entries(window.appData)) { let sum = 0; items.forEach(i => { sum += calculateToOrder(i); }); labels.push(cat); data.push(sum); }
    window.myChart.data.labels = labels; window.myChart.data.datasets[0].data = data; window.myChart.update();
}
...
function setDayFilter(day) {
    ...
    document.querySelectorAll('#day-filter-bar button').forEach(btn => {
        btn.className = btn.id === `day-filter-${day}` ? "px-2.5 py-1 rounded-lg bg-blue-600 text-white border border-blue-500 shadow-sm" : "px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300";
    });
    renderApp();
}
    ...
    for (const [catName, items] of Object.entries(window.appData)) {
        ...
        if (window.viewMode === 'grid') {
            ...
            filtered.forEach(item => {
                const toOrder = calculateToOrder(item);
                rowsHtml += `
                    <tr class="border-b dark:border-slate-700 text-xs font-bold hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition">
                        ...
                        <td class="p-3 data-lastmonth text-center text-sm font-black">${item.orderedLastMonth || 0}</td>
                        ...
                        <td class="p-3 text-slate-400 font-bold truncate max-w-[120px]">${item.days || '-'}</td>
                        <td class="p-3 text-slate-400 font-medium max-w-xs truncate">${item.notes || '-'}</td>
                        <td class="p-3 text-center"><div class="flex justify-center gap-2"><button onclick="openProductModal('${catName}', ${item.originalIndex})" class="text-blue-500">✏️</button><button onclick="deleteProductComplete('${catName}', ${item.originalIndex})" class="text-red-400">🗑️</button></div></td>
                    </tr>
                `;
            });
            ...
            tableWrapper.innerHTML = `
                <table class="w-full text-right border-separate border-spacing-0 custom-table">
                    <thead>
                        <tr class="text-[11px] font-black text-slate-500 bg-slate-50 dark:bg-slate-700/50">
                            <th class="p-3">שם המצרך</th>
                            <th class="p-3 text-center">קיים במזווה</th>
